...
pub fn main() !void {
    ...
    const part_1 = countInvalid(ranges.items);
...
fn countInvalid(ranges: []const Range) usize {
...
fn isInvalid(number: usize) bool {
    ...
    for (1..l + 1) |i| {
        ...
        if (remainder != 0 and std.math.log10_int(remainder) == i - 1) {
            const reproduced = remainder + denominator * remainder;
...
test "isInvalid 101" {
    try std.testing.expect(!isInvalid(101));
}
...
test "not isInvalid three times" {
    try std.testing.expect(!isInvalid(222));
}

test "not isInvalid" {
    try std.testing.expect(!isInvalid(1223));
}

test "not isInvalid long" {
    try std.testing.expect(!isInvalid(1234512344));
}
...
test countInvalid {
    ...
    try std.testing.expectEqual(1227775554, countInvalid(&ranges));
}
